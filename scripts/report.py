      ...
      <table>
        <thead>
          <tr>
            ...
            <th style="text-align:right">Haftalık Getiri</th>
            <th style="text-align:right">Aylık Getiri</th>
            <th style="text-align:right">Yıllık Getiri</th>
...
<style>
  :root {{
    --bg: #eeeaf6; --card: #ffffff; --border: #ded8ef; --text: #443f5e;
  ...
  * {{ box-sizing: border-box; }}
  ...
  .stat {{
    ...
    padding: 14px 18px; min-width: 150px; box-shadow: 0 2px 7px rgba(68,63,94,0.10);
  }}
  .stat .label {{ color: var(--muted); font-size: 12px; margin-bottom: 6px; }}
  .stat .value {{ font-size: 20px; font-weight: 600; }}
  .card {{
    background: var(--card); border: 1px solid var(--border); border-radius: 12px;
    padding: 20px; margin-bottom: 20px; box-shadow: 0 2px 7px rgba(68,63,94,0.10);
  }}
  ...
  .section-card > *:last-child {{ margin-bottom: 0 !important; }}
...
<body>
  ...
  <div class="card section-card">
    <h2>Portföy Yönetim Firmaları</h2>
    <div class="toc">
...
<script>
  ...
  function render() {{
    ...
    const computed = FUNDS.map(f => {{
      const {{ ret, fiyat }} = computeForPeriod(f.hist, days);
      const retG = computeForPeriod(f.hist, PERIOD_DAYS.gunluk).ret;
      const retH = computeForPeriod(f.hist, PERIOD_DAYS.haftalik).ret;
      const retA = computeForPeriod(f.hist, PERIOD_DAYS.aylik).ret;
      const retY = computeForPeriod(f.hist, PERIOD_DAYS.yillik).ret;
      return {{ kod: f.kod, ad: f.ad, sirket: f.sirket, risk: f.risk, ret, fiyat, retG, retH, retA, retY, platform: f.platform, alisValor: f.alisValor, satisValor: f.satisValor, valorKaynak: f.valorKaynak, valorUrl: f.valorUrl }};
    }});
